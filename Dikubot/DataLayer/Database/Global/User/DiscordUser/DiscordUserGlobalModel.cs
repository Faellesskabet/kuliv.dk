...
public class DiscordUserGlobalModel : MainModel
{
    ...
    [BsonElement("Calendar")] public HashSet<Guid> Calendar { get; set; }
    ...
    [BsonElement("CultureInfo")] public string CultureInfo { get; set; } = "en-US";

    public SocketGuild GetSelectedGuild()
    {
        SocketGuild guild = DiscordBot.ClientStatic.GetGuild(SelectedGuild);
        if (guild == null) throw new Exception($"SelectedGuild ({SelectedGuild}) can't be found");
        return guild;
    }
}
...
public abstract class MongoService<TModel> where TModel : MainModel
{
    ...
    {
        _database = database;
        _collection = GetCollection(databaseName);
    }
    ...
    protected IMongoCollection<TModel> GetCollection(string databaseName, MongoCollectionSettings collectionSettings)
    {
    ...
    {
        return Get(model => model.Id == id);
    }

    /// <Summary>Retrieves a element in the collection based on a custom filter</Summary>
    /// <param name="filter">
    ///     The filter is what determines what is returned. Example of a  filter is: (model =>
    ///     model.Id == id)
    /// </param>
    /// <return>A Model.</return>
    ...
    {
        return Get(_ => true, count, 0, null);
    }

    public List<TModel> Get(int count, Expression<Func<TModel, object>> sortBy)
    {
        return Get(_ => true, count, 0, sortBy);
    }

    /// <summary>
    ///     Get the nth first element of page count. The pages have the size of count. So if page = 3, and count = 10, then
    ///     it'll skip the 30 first elements.
    /// </summary>
    /// <param name="count">The maximal amount of elements that will be returned.</param>
    ...
    {
        return Get(filter, count, page, null);
    }
    ...
    /// <returns></returns>
    public List<TModel> Get(Expression<Func<TModel, bool>> filter, int count, int page,
        Expression<Func<TModel, object>> sortBy)
    {
        try
        {
            if (sortBy != null) return _collection.Find(filter).SortBy(sortBy).Skip(count * page).Limit(count).ToList();
    ...
    public virtual List<TModel> GetAll(Expression<Func<TModel, bool>> filter, Expression<Func<TModel, object>> sortBy)
    {
        try
        {
            if (sortBy != null) return _collection.Find(filter).SortBy(sortBy).ToList();
    ...
    /// <Summary>
    ///     Inserts a Model in the collection. If a model with the same ID already exists, then we simply
    ///     invoke Update() on the model instead.
    /// </Summary>
    ...
    ///     MongoDB provides three ways to perform aggregation: the aggregation pipeline, the map-reduce function, and single
    ...
    {
        return _collection.Aggregate(aggregateOptions);
    }

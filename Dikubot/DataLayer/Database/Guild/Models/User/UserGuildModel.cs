...
{
    private HashSet<UserRoleModel> _roles = new();
    ...
    [BsonElement("GroupIds")] public HashSet<Guid> GroupIds { get; set; }
    ...
    /// <summary>
    ///     Tells us whether or not a UserRoleModel is active. By active, it means that has started and not expired. This is
    ///     only relevant for UserRoles which has specified an end- and or startdate
    /// </summary>
    ...
    /// <summary>
    ///     AddRole adds the role to a HashSet of roles. This means no duplicates are allowed. If the role is already present,
    ///     then it is overwritten by the new role
    /// </summary>
    ...
    {
        if (_roles.Contains(userRoleModel)) RemoveRole(userRoleModel);

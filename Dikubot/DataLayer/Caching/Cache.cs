...
/// <summary>
///     This cache is unreliable in the sense that items are not cached individually, meaning items are cached between x
///     and y minutes, instead of a constant amount of time.
///     The reason the cache is implemented like this is really just because I thought it was a fun way of implementing
///     cache
/// </summary>
public class Cache<TKey, TValue> : CronTask
{
    ...
    private readonly Dictionary<TKey, TValue> activeStorage = new();
    private CronTask _cronTask;
    private Scheduler _scheduler = new();
    private Dictionary<TKey, TValue> inactiveStorage = new();
    ...
    ///     The active storage is the storage items is added to, until the next swap, while the inactive storage gets deleted
    ///     at the next swap
    /// </summary>
    private void Swap()
    {
    ...
    {
        TValue value;
        if (activeStorage.TryGetValue(key, out value)) return value;
    ...
    public void Remove(TKey key)
    {
        ...
        if (inactiveStorage.ContainsKey(key)) inactiveStorage.Remove(key);
    }
